...
$domainLabel = $verificationCtrl->getDomainLabel($role);
$errorMsg = null;
$successMsg = null;
$emailSent = false;
$savedEmail = $_SESSION['registro_email_inst'] ?? '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'send_code') {
        ...
        $validation = $verificationCtrl->validateInstitutionalEmail($email, $role);
        if (!$validation['success']) {
            $errorMsg = $validation['message'];
        } else {
            // Enviar código
            $result = $verificationCtrl->sendVerificationCode($email, 'registro');
            if ($result['success']) {
                $_SESSION['registro_email_inst'] = $email;
                $savedEmail = $email;
                $emailSent = true;
                $successMsg = 'Código enviado a ' . $email . '. Revisa tu bandeja de entrada.';
            } else {
        ...
        $email = $_SESSION['registro_email_inst'] ?? '';
        $code = '';
        for ($i = 1; $i <= 6; $i++) {
            $code .= $_POST["code{$i}"] ?? '';
        }
        ...
        if (empty($email)) {
            $errorMsg = 'Sesión expirada. Envía un nuevo código.';
        } else {
            $result = $verificationCtrl->verifyCode($email, $code, 'registro');
            if ($result['success']) {
                // Buscar el usuario recién creado y actualizar email institucional
                require_once __DIR__ . '/../../app/models/Usuario.php';
                $usuarioModel = new Usuario();
                $usuario = $usuarioModel->getByUsuario($cred['usuario']);
                if ($usuario) {
                    $verificationCtrl->verifyRegistrationEmail($usuario['id'], $email);
                }

                // Limpiar email temporal
                unset($_SESSION['registro_email_inst']);

                // Redirigir a credenciales exitosas
                header('Location: credentials-success.php');
                exit;
            } else {
                $errorMsg = $result['message'];
        ...
        if (!empty($email)) {
            ...
            $errorMsg = 'No hay email registrado. Ingresa tu correo primero.';
        }
    }
}
?>
    ...
    <div class="container py-4 py-md-5">
      ...
      <section class="auth-wizard-card mx-auto">
        ...
        <header class="text-center auth-wizard-header">
          ...
          <h1 class="auth-wizard-title mb-2">Verificar Correo Institucional</h1>
          <p class="auth-wizard-subtitle mb-3">
            Confirma tu identidad con tu correo institucional
          </p>
        ...
        <div class="auth-wizard-body">
          <div class="mb-4">
            ...
            <p class="auth-section-subtitle mb-0">
              Ingresa tu correo institucional <strong>@<?php echo htmlspecialchars($domainLabel); ?></strong>
            </p>
          </div>
            ...
            <form method="POST" class="needs-validation" autocomplete="off">
              ...
              <div class="mb-3">
                ...
                <input class="form-control auth-input"
                       type="email"
                       name="email"
                       placeholder="tu.correo@<?php echo htmlspecialchars($domainLabel); ?>"
                       value="<?php echo htmlspecialchars($savedEmail); ?>"
                       required />
                <div class="auth-help mt-2">
                  <?php if ($role === 'egresado'): ?>
                    Usa tu correo de alumno: @alumno.utpuebla.edu.mx
                  <?php else: ?>
                    Usa tu correo institucional: @utpuebla.edu.mx
                  <?php endif; ?>
                </div>
              </div>
          ...
          <div id="verifyCodeSection" class="<?php echo $emailSent ? '' : 'd-none'; ?>">
            <div class="text-center mb-4">
              ...
              <p class="auth-section-subtitle">
                ...
                <strong><?php echo htmlspecialchars($savedEmail); ?></strong>
              </p>
            </div>
            ...
            <form method="POST" class="needs-validation" autocomplete="off" id="verifyForm">
              ...
              <div class="d-flex gap-2 auth-actions">
                <button class="btn btn-light auth-btn-back" type="button" id="btnChangeEmail">
                  <i class="bi bi-chevron-left me-1"></i> Cambiar Email
                </button>
